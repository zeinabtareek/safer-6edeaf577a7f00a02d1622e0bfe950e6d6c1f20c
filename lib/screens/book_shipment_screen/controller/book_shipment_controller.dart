...
class BookShipmentController extends GetxController {
  ...
  validation(
    ...
    if (widthController.text == '' ||
      ...
      showCustomSnackBar(
          message: 'make_sure_to_choose_wight'.tr,
          isError: true);
    } else {
      calculate(
  ...
  bookTrip({required Trips tripDetails, movingDate}) async {
    if (totalPrice.value == 0.0 ||
      ...
      showCustomSnackBar(message: 'calculate_cost_first'.tr, isError: true);
    } else {
  ...
  Future<void> addTicketToFirestore(Trips tripDetails, movingDate,
    ...
    if (docId != null) {
      ...
      showCustomSnackBar(
          message: 'shipment_booked_success'.tr,
          isError: false,
          isInfo: false);
      Get.back(); // Assuming you're using the Get package for navigation
    } else {
      showCustomSnackBar(message: 'error_book_shipping'.tr);
  ...
  Future<void> sendNotification({token}) async {
    ...
    final data = {
      "data": {
        "message": "Accept shipment Request",
        "title": "This is shipment  Request",
      },
      "to":token
